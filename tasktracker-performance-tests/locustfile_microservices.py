"""
Locust performance test for Microservices Architecture
Tests the microservices TaskTracker application on port 8000 (API Gateway)
"""
...
ARCH_CONFIG = ARCHITECTURES["microservices"]
...
class TaskTrackerUser(HttpUser):
    """
    Simulates a user interacting with the TaskTracker microservices application.
    
    This user performs realistic operations through the API Gateway:
    - Logs in with existing credentials (User Service)
    - Creates, reads, updates, and deletes tasks (Task Service)
    - Checks statistics (Stats Service)
    ...
    def get_stats(self):
        """Get user statistics (tests Stats Service calling Task Service)."""
        self.client.get(
            f"{API_PREFIX}/stats/",
            headers=self.headers,
