#!/bin/bash
# Run performance tests for Monolithic Architecture

echo "=========================================="
echo "MONOLITHIC ARCHITECTURE PERFORMANCE TEST"
echo "=========================================="
echo ""

# Check if applications are running
echo "Checking if Monolithic app is running on port 9000..."
if ! curl -s http://localhost:9000/health > /dev/null; then
    echo "❌ ERROR: Monolithic app is not running on port 9000"
    echo "Please start it first with:"
    echo "  cd ../tasktracker-mono && docker compose up -d"
    exit 1
fi

echo "✓ Monolithic app is running"
echo ""
...
RESULTS_DIR="results/monolithic_${TIMESTAMP}"
mkdir -p "$RESULTS_DIR"
